...
void gemmblock_5_16_v2(int M, int N, int K,int kmin, int kmax, int m, int n, const float * A, const int lda, const float * B, const int ldb, float * C, const int ldc) {
  ...
  for(int k = kmin; k < kmax; k++) {
    ...
    __m512 a00 = _mm512_set1_ps(*(abase + 0x00 * K));
    __m512 a10 = _mm512_set1_ps(*(abase + 0x01 * K));
    __m512 a20 = _mm512_set1_ps(*(abase + 0x02 * K));
    __m512 a30 = _mm512_set1_ps(*(abase + 0x03 * K));
    __m512 a40 = _mm512_set1_ps(*(abase + 0x04 * K));
...
//calc [5,16] of C
//get a row of A to vector, get a col of B to vector, reduce vector to a float finally 
